 ...
 * and you have to choose only one of the three construction methods. \n
 *
 * Note that the returned {@link OH_NNCompilation} instance only saves the
 * <b>modelBuffer</b> pointer inside, instead of copying its data.
 ...
 *
 * Some devices have their own specific attributes which have not been opened in NNRt.
 * This method provides an additional way for you to set these custom hardware attributes of the device.
 * You should query their names and values from the device vendor's documents,
 * and add them into compilation instance one by one. These attributes will be passed directly to device driver,
 * and this method will return error code if the driver cannot parse them. \n
 *
 ...
 * Reads the cache file in the path and passes the data to the underlying
 * device for conversion into executable model instances. \n
 *
 * - A complete cache file exists in the cache directory, and its version is earlier than <b>version</b>:
 * When model compilation is complete on the underlying device,
 * overwrites the cache file and changes the version number to <b>version</b>. \n
 *
 ...
 *
 * @param compilation Pointer to the {@link OH_NNCompilation} instance.
 * @param cachePath Directory for storing model cache files. This method creates directories for different devices in
 *                  the <b>cachePath</b> directory. You are advised to use a separate cache directory for each model.
 ...
 *
 * @param compilation Pointer to the {@link OH_NNCompilation} instance.
 * @param enableFloat16 Indicates whether to enable float16. If this parameter is
 *                      set to <b>true</b>, float16 inference is performed.
 ...
 *                    After a compilation instance is destroyed,
 ...
 *
 * This method needs to be called to release the executor instance created by calling
 * {@link OH_NNExecutor_Construct}. Otherwise, the memory leak will occur. \n
 *
...
/**
 * @brief Sets the callback function handle for the post-process when the
 * device driver service is dead during asynchronous execution.
 *
 ...
 * or {@link OH_NNTensor_CreateWithFd}. And then the input tensors data which is got by
 * {@link OH_NNTensor_GetDataBuffer} must be filled. The executor will then yield out the results
 * by inference execution and fill them into output tensors data for you to read. \n
 *
 ...
 * Input and output tensors should be created first by {@link OH_NNTensor_Create}, {@link OH_NNTensor_CreateWithSize}
 * or {@link OH_NNTensor_CreateWithFd}. And then the input tensors data which is got by
 * {@link OH_NNTensor_GetDataBuffer} must be filled. The executor will yield out the results by inference execution
 * and fill them into output tensors data for you to read.\n
 *
 * In the case of dynamic shape, you can get the real output shape directly by {@link OH_NNExecutor_GetOutputShape},
 * or you can create a tensor descriptor from an output tensor by {@link OH_NNTensor_GetTensorDesc},
 * and then read its real shape by {@link OH_NNTensorDesc_GetShape}.\n
