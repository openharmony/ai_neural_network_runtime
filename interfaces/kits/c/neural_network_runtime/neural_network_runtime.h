 ...
 *
 * @brief Defines the Neural Network Runtime APIs. The AI inference framework uses the Native APIs provided
 *        by Neural Network Runtime to construct models.
 * 
 ...
 * {@link OH_NNQuantParam_SetZeroPoints}, {@link OH_NNQuantParam_SetNumBits} to set its attributes,
 * and then call {@link OH_NNModel_SetTensorQuantParams} to set it to a tensor.
 ...
 *
 * The {@link NN_QuantParam} instance needs to be released to avoid memory leak
 * after it is set to a {@link NN_TensorDesc}. \n
 * 
...
/**
 * @brief Creates a model instance of the {@link OH_NNModel} type
 *        and uses other APIs provided by OH_NNModel to construct the model instance.
 *
 * Before composition, call {@link OH_NNModel_Construct} to create a model instance. Based on the model topology,
 * call the {@link OH_NNModel_AddTensorToModel}, {@link OH_NNModel_AddOperation},
 * and {@link OH_NNModel_SetTensorData} methods to fill in the data and operator nodes of the model,
 * and then call {@link OH_NNModel_SpecifyInputsAndOutputs} to specify the inputs and outputs of the model.
 * After the model topology is constructed, call {@link OH_NNModel_Finish} to build the model. \n
 *
 * After a model instance is no longer used,
 * you need to destroy it by calling {@link OH_NNModel_Destroy} to avoid memory leak. \n
 *
 ...
 * This method is used to add tensors to a model instance based on the <b>tensorDesc</b> parameter
 * with type of {@link NN_TensorDesc}. {@link NN_TensorDesc} contains some attributes such as shape,
 * format, data type and provides corresponding APIs to access them.
 * The order of adding tensors is specified by the indices recorded in the model.
 ...
 *
 * Neural Network Runtime supports inputs and outputs of the dynamic shape.
 * When adding a data node with a dynamic shape,
 * you need to set the dimensions that support dynamic changes to <b>-1</b>.
 * For example, if the shape of a four-dimensional tensor is set to <b>[1, -1, 2, 2]</b>,
 ...
 *                   The tensor descriptor specifies the attributes of the tensor added to the model instance.
 * @return Execution result of the function. If the operation is successful, <b>OH_NN_SUCCESS</b> is returned.
 *         If the operation fails, an error code is returned. For details about the error codes,
 *         see {@link OH_NN_ReturnCode}.
 * @since 11
 ...
 * the operator parameters, inputs, and outputs are specified by <b>paramIndices</b>, <b>inputIndices</b>,
 * and <b>outputIndices</b> respectively.
 ...
 * For details about the expected parameters, input attributes, and output attributes of each operator,
 ...
 *
 * @param model Pointer to the {@link OH_NNModel} instance.
 * @param op Specifies the type of an operator to be added. For details,
 *           see the enumerated values of {@link OH_NN_OperationType}.
 ...
 *
 * A tensor must be specified as the end-to-end inputs and outputs of a model instance.
 * This type of tensor cannot be set using {@link OH_NNModel_SetTensorData}. \n
 *
 * The index of a tensor is determined by the order in which the tensor is added to the model.
 * For details about how to add a tensor, see {@link OH_NNModel_AddTensorToModel}. \n
 *
 ...
 *
 * After the model topology is set up, call this method to indicate that the composition is complete.
 * After this method is called, additional composition operations cannot be performed.
 * If {@link OH_NNModel_AddTensorToModel}, {@link OH_NNModel_AddOperation},
 ...
 *
 * This method needs to be called to release the model instance created by calling {@link OH_NNModel_Construct}.
 * Otherwise, memory leak will occur. \n
 *
 * If <b>model</b> or <b>*model</b> is a null pointer,
 * this method only prints warning logs and does not execute the release. \n
 *
 * @param model Double pointer to the {@link OH_NNModel} instance. After a model instance is destroyed,
 *              this method sets <b>*model</b> to a null pointer.
 * @since 9
 * @version 1.0
 */
...
/**
 * @brief Queries whether the device supports operators in the model.
 *        The support status is indicated by the Boolean value.
 *
 * Queries whether underlying device supports operators in a model instance. The device is specified by
 * <b>deviceID</b>, and the result is represented by the array pointed by <b>isSupported</b>.
 * If the <i>i</i>th operator is supported, the value of <b>(*isSupported)</b>[<i>i</i>] is <b>true</b>.
 * Otherwise, the value is <b>false</b>. \n
 *
 * After this method is successfully executed, <b>(*isSupported)</b> points to the bool array that records the operator
 * support status. The operator quantity for the array length is the same as that for the model instance.
 * The memory corresponding to this array is managed by Neural Network Runtime and is automatically destroyed
 * after the model instance is destroyed or this method is called again. \n
 *
 * @param model Pointer to the {@link OH_NNModel} instance.
 * @param deviceID Device ID to be queried, which can be obtained by using {@link OH_NNDevice_GetAllDevicesID}.
 * @param isSupported Pointer to the bool array. When this method is called, <b>(*isSupported)</b> must be
 *                    a null pointer. Otherwise, {@link OH_NN_INVALID_PARAMETER} is returned.
 * @param opCount Number of operators in a model instance, corresponding to the length of the
 *                <b>(*isSupported)</b> array.
 * @return Execution result of the function. If the operation is successful, <b>OH_NN_SUCCESS</b> is returned.
 *         If the operation fails, an error code is returned. For details about the error codes,
 *         see {@link OH_NN_ReturnCode}.
 * @since 9
 ...
 * computing result to the shared memory to which the memory instance points.
 * By using this method, concurrent execution of input setting,
 * computing, and read can be implemented to improve inference efficiency of a data flow.\n
 *
