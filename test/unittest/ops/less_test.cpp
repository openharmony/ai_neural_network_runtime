...
namespace UnitTest {
class LessBuilderTest : public OpsTest {
public:
    void SetUp() override;
    void TearDown() override;

protected:
    LessBuilder m_builder;
    std::vector<uint32_t> m_inputs {0, 1};
    std::vector<uint32_t> m_outputs {2};
    std::vector<int32_t> m_dim {1, 2, 2, 1};
};

void LessBuilderTest::SetUp() {}

void LessBuilderTest::TearDown() {}

/**
 * @tc.name: less_build_001
 * @tc.desc: Verify that the build function returns a successful message.
 * @tc.type: FUNC
 */
HWTEST_F(LessBuilderTest, less_build_001, TestSize.Level1)
{
 ...
 * @tc.type: FUNC
 */
HWTEST_F(LessBuilderTest, less_build_003, TestSize.Level1)
{
    m_inputs = {0, 1, 2};
    m_outputs = {3};
...
/**
 * @tc.name: less_getprimitive_001
 * @tc.desc: Verify that the getPrimitive function returns a successful message
 * @tc.type: FUNC
 */
HWTEST_F(LessBuilderTest, less_getprimitive_001, TestSize.Level1)
{
    SaveInputTensor(m_inputs, OH_NN_INT32, m_dim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_INT32, m_dim, nullptr);
...
/**
 * @tc.name: less_getprimitive_002
 * @tc.desc: Verify that the getPrimitive function returns a failed message without build.
 * @tc.type: FUNC
 */
HWTEST_F(LessBuilderTest, less_getprimitive_002, TestSize.Level1)
{
    LiteGraphPrimitvePtr primitive = m_builder.GetPrimitive();
    LiteGraphPrimitvePtr expectPrimitive(nullptr, DestroyLiteGraphPrimitive);
