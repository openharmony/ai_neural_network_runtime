...
namespace UnitTest {
class FlattenBuilderTest : public OpsTest {
public:
    void SetUp() override;
...
void FlattenBuilderTest::SetUp() {}

void FlattenBuilderTest::TearDown() {}

void FlattenBuilderTest::SaveParamsTensor(OH_NN_DataType dataType,
    const std::vector<int32_t> &dim, const OH_NN_QuantParam* quantParam, OH_NN_TensorType type)
{
...
HWTEST_F(FlattenBuilderTest, flatten_build_001, TestSize.Level1)
{
    ...
    SaveParamsTensor(OH_NN_INT64, m_paramDim, nullptr, OH_NN_FLATTEN_AXIS);
...
/**
 * @tc.name: flatten_getprimitive_001
...
HWTEST_F(FlattenBuilderTest, flatten_getprimitive_001, TestSize.Level1)
{
    ...
    auto returnValue = mindspore::lite::MindIR_Flatten_GetAxis(primitive.get());
    EXPECT_EQ(returnValue, axisValue);
}

/**
 * @tc.name: flatten_getprimitive_002
