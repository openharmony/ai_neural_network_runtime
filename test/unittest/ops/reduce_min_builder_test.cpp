...
#include "ops/reducemin_builder.h"
...
namespace UnitTest {
class ReduceMinBuilderTest : public OpsTest {
public:
    void SetUp() override;
...
void ReduceMinBuilderTest::SetUp() {}

void ReduceMinBuilderTest::TearDown() {}

void ReduceMinBuilderTest::SetKeepDims(OH_NN_DataType dataType,
    const std::vector<int32_t> &dim, const OH_NN_QuantParam* quantParam, OH_NN_TensorType type)
{
...
void ReduceMinBuilderTest::SetCoeff(OH_NN_DataType dataType,
    const std::vector<int32_t> &dim, const OH_NN_QuantParam* quantParam, OH_NN_TensorType type)
{
...
void ReduceMinBuilderTest::SetReduceToEnd(OH_NN_DataType dataType,
    const std::vector<int32_t> &dim, const OH_NN_QuantParam* quantParam, OH_NN_TensorType type)
{
...
/**
 * @tc.name: reducemin_build_002
 * @tc.desc: Call Build func twice to verify the abnormal behavior of the Build function
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_build_002, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_BOOL, m_inputDim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_BOOL, m_outputDim, nullptr);
    SetKeepDims(OH_NN_BOOL, m_paramDim, nullptr, OH_NN_REDUCE_MIN_KEEP_DIMS);
 ...
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_build_003, TestSize.Level0)
{
    m_inputs = {0, 1, 2};
    m_outputs = {3};
    m_params = {4, 5, 6};
...
/**
 * @tc.name: reducemin_build_005
 * @tc.desc: Verify that the build function return a failed message with null allTensor
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_build_005, TestSize.Level0)
{
...
/**
 * @tc.name: reducemin_build_006
 * @tc.desc: Verify that the build function return a failed message without output tensor
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_build_006, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_BOOL, m_inputDim, nullptr);
...
HWTEST_F(ReduceMinBuilderTest, reducemin_build_009, TestSize.Level0)
{
    ...
    std::shared_ptr<NNTensor> reduceToEndTensor = TransToNNTensor(OH_NN_INT64,
        m_paramDim, nullptr, OH_NN_REDUCE_MIN_REDUCE_TO_END);
...
/**
 * @tc.name: reducemin_build_016
 * @tc.desc: Verify that the build function return a failed message with empty keepdims's buffer
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_build_016, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_BOOL, m_inputDim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_BOOL, m_outputDim, nullptr);
...
/**
 * @tc.name: reducemin_build_017
 * @tc.desc: Verify that the build function return a failed message with empty coeff's buffer
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_build_017, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_BOOL, m_inputDim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_BOOL, m_outputDim, nullptr);
...
/**
 * @tc.name: reducemin_get_primitive_001
 * @tc.desc: Verify the GetPrimitive function return nullptr
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_get_primitive_001, TestSize.Level0)
{
...
/**
 * @tc.name: reducemin_get_primitive_002
 * @tc.desc: Verify the normal params return behavior of the getprimitive function
 * @tc.type: FUNC
 */
HWTEST_F(ReduceMinBuilderTest, reducemin_get_primitive_002, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_BOOL, m_inputDim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_BOOL, m_outputDim, nullptr);
    SetKeepDims(OH_NN_BOOL, m_paramDim, nullptr, OH_NN_REDUCE_MIN_KEEP_DIMS);
    ...
    EXPECT_EQ(OH_NN_SUCCESS, m_builder.Build(m_params, m_inputsIndex, m_outputsIndex, m_allTensors));
    LiteGraphTensorPtr reduceminPrimitive = m_builder.GetPrimitive();
    LiteGraphTensorPtr expectPrimitive = {nullptr, DestroyLiteGraphPrimitive};

    EXPECT_NE(reduceminPrimitive, expectPrimitive);
    auto returnKeepDimsValue = mindspore::lite::MindIR_ReduceFusion_GetKeepDims(reduceminPrimitive.get());
    EXPECT_EQ(returnKeepDimsValue, keepDimsValue);
    auto returnCoeffValue = mindspore::lite::MindIR_ReduceFusion_GetCoeff(reduceminPrimitive.get());
    EXPECT_EQ(returnCoeffValue, coeffValue);
    auto returnReduceToEndValue = mindspore::lite::MindIR_ReduceFusion_GetReduceToEnd(reduceminPrimitive.get());
