 ...
 * @tc.type: FUNC
 */
HWTEST_F(Relu6BuilderTest, relu6_build_003, TestSize.Level0)
{
    m_inputs = {0, 1};
    m_outputs = {2};
...
HWTEST_F(Relu6BuilderTest, relu6_build_007, TestSize.Level0)
{
    ...
    SaveInputTensor(m_inputs, OH_NN_FLOAT32, m_dim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_FLOAT32, m_dim, nullptr);
    std::shared_ptr<NNTensor> relu6Tensor = TransToNNTensor(OH_NN_INT32, paramDim, nullptr, OH_NN_TENSOR);
    m_allTensors.emplace_back(relu6Tensor);
...
/**
 * @tc.name: relu6_get_primitive_001
 * @tc.desc: Verify the GetPrimitive function return nullptr
 * @tc.type: FUNC
 */
HWTEST_F(Relu6BuilderTest, relu6_get_primitive_001, TestSize.Level0)
{
...
/**
 * @tc.name: relu6_get_primitive_002
 * @tc.desc: Verify the normal params return behavior of the getprimitive function
 * @tc.type: FUNC
 */
HWTEST_F(Relu6BuilderTest, relu6_get_primitive_002, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_FLOAT32, m_dim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_FLOAT32, m_dim, nullptr);

    EXPECT_EQ(OH_NN_SUCCESS, m_builder.Build(m_paramsIndex, m_inputsIndex, m_outputsIndex, m_allTensors));
    LiteGraphTensorPtr relu6Primitive = m_builder.GetPrimitive();
    LiteGraphTensorPtr expectPrimitive = {nullptr, DestroyLiteGraphPrimitive};
    EXPECT_NE(relu6Primitive, expectPrimitive);

    mindspore::lite::ActivationType activationType = mindspore::lite::ACTIVATION_TYPE_RELU6;
    auto returnValue = mindspore::lite::MindIR_Activation_GetActivationType(relu6Primitive.get());
