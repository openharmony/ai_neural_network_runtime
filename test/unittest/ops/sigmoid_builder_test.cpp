...
namespace UnitTest {
class SigmoidBuilderTest : public OpsTest {
public:
    void SetUp() override;
    void TearDown() override;

protected:
    SigmoidBuilder m_builder;
    std::vector<uint32_t> m_inputs {0};
    std::vector<uint32_t> m_outputs {1};
    std::vector<int32_t> m_dim {1, 5, 1, 1};
};

void SigmoidBuilderTest::SetUp() {}

void SigmoidBuilderTest::TearDown() {}

/**
 * @tc.name: sigmoid_build_001
 * @tc.desc: Provide normal input, output, and parameters to verify the normal behavior of the Build function
 * @tc.type: FUNC
 */
HWTEST_F(SigmoidBuilderTest, sigmoid_build_001, TestSize.Level0)
{
...
/**
 * @tc.name: sigmoid_build_002
 * @tc.desc: Call Build func twice to verify the abnormal behavior of the Build function
 * @tc.type: FUNC
 */
HWTEST_F(SigmoidBuilderTest, sigmoid_build_002, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_FLOAT32, m_dim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_FLOAT32, m_dim, nullptr);
...
/**
 * @tc.name: sigmoid_build_005
 * @tc.desc: Verify that the build function return a failed message with null allTensor
 * @tc.type: FUNC
 */
HWTEST_F(SigmoidBuilderTest, sigmoid_build_005, TestSize.Level0)
{
...
/**
 * @tc.name: sigmoid_build_006
 * @tc.desc: Verify that the build function return a failed message without output tensor
 * @tc.type: FUNC
 */
HWTEST_F(SigmoidBuilderTest, sigmoid_build_006, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_FLOAT32, m_dim, nullptr);
...
HWTEST_F(SigmoidBuilderTest, sigmoid_build_007, TestSize.Level0)
{
    ...
    SaveInputTensor(m_inputs, OH_NN_FLOAT32, m_dim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_FLOAT32, m_dim, nullptr);
    std::shared_ptr<NNTensor> sigmoidTensor = TransToNNTensor(OH_NN_INT32, paramDim, nullptr, OH_NN_TENSOR);
    m_allTensors.emplace_back(sigmoidTensor);
...
/**
 * @tc.name: sigmoid_get_primitive_001
 * @tc.desc: Verify the GetPrimitive function return nullptr
 * @tc.type: FUNC
 */
HWTEST_F(SigmoidBuilderTest, sigmoid_get_primitive_001, TestSize.Level0)
{
...
/**
 * @tc.name: sigmoid_get_primitive_002
 * @tc.desc: Verify the normal params return behavior of the getprimitive function
 * @tc.type: FUNC
 */
HWTEST_F(SigmoidBuilderTest, sigmoid_get_primitive_002, TestSize.Level0)
{
    SaveInputTensor(m_inputs, OH_NN_FLOAT32, m_dim, nullptr);
    SaveOutputTensor(m_outputs, OH_NN_FLOAT32, m_dim, nullptr);

    EXPECT_EQ(OH_NN_SUCCESS, m_builder.Build(m_paramsIndex, m_inputsIndex, m_outputsIndex, m_allTensors));
    LiteGraphTensorPtr sigmoidPrimitive = m_builder.GetPrimitive();
    LiteGraphTensorPtr expectPrimitive = {nullptr, DestroyLiteGraphPrimitive};
    EXPECT_NE(sigmoidPrimitive, expectPrimitive);

    mindspore::lite::ActivationType activationType = mindspore::lite::ACTIVATION_TYPE_SIGMOID;
    auto returnValue = mindspore::lite::MindIR_Activation_GetActivationType(sigmoidPrimitive.get());
