...
namespace UnitTest {
class ModBuilderTest : public OpsTest {
public:
    void SetUp() override;
...
protected:
    ModBuilder m_builder;
    std::vector<uint32_t> m_inputs {0, 1};
    std::vector<uint32_t> m_outputs {2};
    std::vector<int32_t> m_dim {1, 2, 2, 1};
};

void ModBuilderTest::SetUp() {}

void ModBuilderTest::TearDown() {}

void ModBuilderTest::SetInputTensor()
{
    m_inputsIndex = m_inputs;
    std::shared_ptr<NNTensor> input1Tensor;
    input1Tensor = TransToNNTensor(OH_NN_INT32, m_dim, nullptr, OH_NN_TENSOR);
    m_allTensors.emplace_back(input1Tensor);

    std::shared_ptr<NNTensor> input2Tensor;
    input2Tensor = TransToNNTensor(OH_NN_INT32, m_dim, nullptr, OH_NN_TENSOR);
    m_allTensors.emplace_back(input2Tensor);
}

/**
 * @tc.name: mod_build_001
