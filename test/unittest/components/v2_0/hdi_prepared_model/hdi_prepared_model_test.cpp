...
OH_NN_ReturnCode HDIPreparedModelTest::Run(std::vector<IOTensor>& inputs)
{
    ...
    OHOS::sptr<V2_0::MockIPreparedModel> sp =
        OHOS::sptr<V2_0::MockIPreparedModel>(new (std::nothrow) V2_0::MockIPreparedModel());
    EXPECT_NE(sp, nullptr);

    std::unique_ptr<HDIPreparedModelV2_0> preparedModel = std::make_unique<HDIPreparedModelV2_0>(sp);
...
{
    std::vector<V2_0::SharedBuffer> bufferVect = {{100, 100, 0, 100}};
    OHOS::sptr<V2_0::IPreparedModel> hdiPreparedModel =
        OHOS::sptr<V2_0::MockIPreparedModel>(new (std::nothrow) V2_0::MockIPreparedModel());
    std::unique_ptr<HDIPreparedModelV2_0> preparedModel = std::make_unique<HDIPreparedModelV2_0>(hdiPreparedModel);
...
{
    std::vector<V2_0::SharedBuffer> bufferVect;
    OHOS::sptr<V2_0::IPreparedModel> mockPreparedModel =
        OHOS::sptr<V2_0::MockIPreparedModel>(new (std::nothrow) V2_0::MockIPreparedModel());
...
HWTEST_F(HDIPreparedModelTest, hidpreparedmodel_run_001, TestSize.Level0)
{
    ...
    std::shared_ptr<V2_0::MockIPreparedModel> sp = std::make_shared<V2_0::MockIPreparedModel>();
    OHOS::sptr<V2_0::IPreparedModel> hdiPreparedModel =
        OHOS::sptr<V2_0::MockIPreparedModel>(new (std::nothrow) V2_0::MockIPreparedModel());
    EXPECT_NE(hdiPreparedModel, nullptr);
