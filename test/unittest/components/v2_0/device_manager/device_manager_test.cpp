...
protected:
    void MockInit(OHOS::sptr<V2_0::MockIDevice> device, const std::vector<int32_t>& typeVect,
        const std::string& deviceName, const std::string& vendorName);
};

void DeviceManagerTest::MockInit(OHOS::sptr<V2_0::MockIDevice> device, const std::vector<int32_t>& typeVect,
    const std::string& deviceName, const std::string& vendorName)
{
...
HWTEST_F(DeviceManagerTest, devicemanager_registerdevice_001, TestSize.Level0)
{
    ...
    OHOS::sptr<V2_0::MockIDevice> device = OHOS::sptr<V2_0::MockIDevice>(new (std::nothrow) V2_0::MockIDevice());
    ...
    std::function<std::shared_ptr<HDIDeviceV2_0>()> creator =
...
{
    std::function<std::shared_ptr<HDIDeviceV2_0>()> creator =
        []()->std::shared_ptr<HDIDeviceV2_0> {return nullptr;};
    auto& deviceManager = DeviceManager::GetInstance();
    OH_NN_ReturnCode result = deviceManager.RegisterDevice(creator);
...
HWTEST_F(DeviceManagerTest, devicemanager_registerdevice_005, TestSize.Level0)
{
    ...
    const size_t expectDeviceId {std::hash<std::string> {} ("MockDeviceA_MockVendorA_v1_0")};
    EXPECT_EQ(expectDeviceId, idVect[0]);

    const std::string expectDeviceName = "MockDeviceA_MockVendorA_v1_0";
