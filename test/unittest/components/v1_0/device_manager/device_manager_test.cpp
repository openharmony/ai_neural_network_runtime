...
HWTEST_F(DeviceManagerTest, devicemanager_registerdevice_001, TestSize.Level0)
{
    ...
    std::function<std::shared_ptr<HDIDeviceV1_0>()> creator =
        [&device]()->std::shared_ptr<HDIDeviceV1_0> {return std::make_shared<HDIDeviceV1_0>(device);};
    auto& deviceManager = DeviceManager::GetInstance();
...
{
    std::function<std::shared_ptr<HDIDeviceV1_0>()> creator =
        []()->std::shared_ptr<HDIDeviceV1_0> {return nullptr;};
    auto& deviceManager = DeviceManager::GetInstance();
    OH_NN_ReturnCode result = deviceManager.RegisterDevice(creator);
