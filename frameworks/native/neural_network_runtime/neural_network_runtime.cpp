    ...
    const mindspore::lite::LiteGraph* pLiteGraph, ExtensionConfig& extensionConfig)
{
    ...
    if (extensionMap.find(EXTENSION_KEY_AIPP) != extensionMap.end()) {
...
{
    if ((cacheDir == nullptr) || (modelName == nullptr)) {
        LOGI("OH_NNModel_HasCache get empty cache directory or empty model name.");
        return false;
    }
...
NNRT_API OH_NN_ReturnCode OH_NN_IsSupportAIPP(bool& support)
{
    ...
    if ((deviceName.substr(CHECK_SUM_ZERO, CHECK_SUM_TWO) == SUPPORT_AIPP_DEVICE.substr(CHECK_SUM_ZERO,
            CHECK_SUM_TWO)) &&  (deviceName.substr(CHECK_SUM_TWO) >= SUPPORT_AIPP_DEVICE.substr(CHECK_SUM_TWO))) {
        support = true;
        return OH_NN_SUCCESS;
    }
    return OH_NN_FAILED;
...
OH_NN_ReturnCode RunSyncWithAipp(Executor *executor,
                                 ...
                                 const char* aippStrings)
{
    ...
    if (ret != OH_NN_SUCCESS) {
        LOGE("OH_NNExecutor_RunSyncWithAipp failed, fail to run executor.");
        return ret;
    }
                                                        ...
                                                        const char* aippString)
{
    ...
    if ((outputCount == 0) || (outputCount > INPUT_OUTPUT_MAX_INDICES)) {
        LOGE("OH_NNExecutor_RunSyncWithAipp failed, outputCount is 0 or more than 200.");
        return OH_NN_INVALID_PARAMETER;
    }
