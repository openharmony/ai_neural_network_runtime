...
OH_NN_ReturnCode LogBuilder::Build(const std::vector<uint32_t>& paramsIndex,
                                   const std::vector<uint32_t>& inputsIndex,
                                   const std::vector<uint32_t>& outputsIndex,
                                   const std::vector<std::shared_ptr<NNTensor>>& allTensors)
{
    if (m_isBuild) {
        LOGE("[Log] Build failed, the log operation has been build. cannot build again.");
...
LiteGraphPrimitvePtr LogBuilder::GetPrimitive()
{
    if (!m_isBuild) {
        LOGE("[Log] GetPrimitive failed, cannot get primitive before call build.");
...
REGISTER_OPS(LogBuilder, OH_NN_OPS_LOG);
} // namespace Ops
} // namespace NeuralNetworkRuntime
} // namespace OHOS
