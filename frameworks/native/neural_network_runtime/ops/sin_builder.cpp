...
OH_NN_ReturnCode SinBuilder::Build(const std::vector<uint32_t>& paramsIndex,
                                   const std::vector<uint32_t>& inputsIndex,
                                   const std::vector<uint32_t>& outputsIndex,
                                   const std::vector<std::shared_ptr<NNTensor>>& allTensors)
{
    if (m_isBuild) {
        LOGE("[Sin] Build failed, the sin operation has been build. cannot build again.");
...
LiteGraphPrimitvePtr SinBuilder::GetPrimitive()
{
    if (!m_isBuild) {
        LOGE("[Sin] GetPrimitive failed, cannot get primitive before call build.");
...
REGISTER_OPS(SinBuilder, OH_NN_OPS_SIN);
} // namespace Ops
} // namespace NeuralNetworkRuntime
} // namespace OHOS
