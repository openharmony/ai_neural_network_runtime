...
std::vector<int8_t> ConvertAddFusion(PrimitivePtr primitive)
{
    ...
    AddFusion addFusion{};
    addFusion.activationType = static_cast<HDI::Nnrt::V1_0::ActivationType>(
        mindspore::lite::MindIR_Activation_GetActivationType(primitive));
...
OHOS::HDI::Nnrt::V1_0::SharedBuffer Copy_MindIR_Tensor_Data_To_HDIBuffer(const TensorPtr tensor,
    const OHOS::HDI::Nnrt::V1_0::SharedBuffer &bufferTemplete, uint8_t *mmapPtr, unsigned int offset)
{
    if (tensor == nullptr) {
        LOGE("MindIR_LiteGraph_To_Model v1 tensor is nullptr.");
        return {-1, 0, offset, 0};
    }
    if (mmapPtr == nullptr) {
